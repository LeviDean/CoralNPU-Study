func.func @main_dispatch_13_matmul_17x64x32_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert>} {
  ...
  %13 = scf.forall (%arg0) = (0) to (17) step (16) shared_outs(%arg1 = %12) -> (tensor<17x64xf32>) {
    ...
    %15 = scf.forall (%arg2) = (0) to (%14) step (16) shared_outs(%arg3 = %extracted_slice) -> (tensor<?x64xf32>) {
      ...
      %17 = scf.forall (%arg4, %arg5) = (0, 0) to (%16, 64) step (8, 8) shared_outs(%arg6 = %extracted_slice_5) -> (tensor<?x64xf32>) {
        %18 = affine.min affine_map<(d0)[s0] -> (-d0 + s0, 8)>(%arg4)[%16]
        %19 = tensor.empty(%18) : tensor<?x8xf32>
        %20 = linalg.fill {stream.affinity = #hal.device.affinity<@__device_0>} ins(%cst_4 : f32) outs(%19 : tensor<?x8xf32>) -> tensor<?x8xf32>
        %21 = scf.for %arg7 = %c0 to %c32 step %c1 iter_args(%arg8 = %20) -> (tensor<?x8xf32>) {
          %23 = affine.apply affine_map<()[s0, s1, s2] -> (s0 + s1 + s2)>()[%arg4, %arg2, %arg0]
          %extracted_slice_8 = tensor.extract_slice %9[%23, %arg7] [%18, 1] [1, 1] : tensor<17x32xf32> to tensor<?x1xf32>
          %extracted_slice_9 = tensor.extract_slice %10[%arg5, %arg7] [8, 1] [1, 1] : tensor<64x32xf32> to tensor<8x1xf32>
          %extracted_slice_10 = tensor.extract_slice %arg8[0, 0] [%18, 8] [1, 1] : tensor<?x8xf32> to tensor<?x8xf32>
          %24 = linalg.matmul indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>] {lowering_config = #iree_cpu.lowering_config<cache_parallel = [16, 0, 0], cache_reduction = [0, 0, 0], distribution = [16, 0, 0], vector_common_parallel = [8, 8, 0], vector_reduction = [0, 0, 1]>, stream.affinity = #hal.device.affinity<@__device_1>} ins(%extracted_slice_8, %extracted_slice_9 : tensor<?x1xf32>, tensor<8x1xf32>) outs(%extracted_slice_10 : tensor<?x8xf32>) -> tensor<?x8xf32>
          %inserted_slice = tensor.insert_slice %24 into %arg8[0, 0] [%18, 8] [1, 1] : tensor<?x8xf32> into tensor<?x8xf32>
          scf.yield %inserted_slice : tensor<?x8xf32>
        }
        %extracted_slice_6 = tensor.extract_slice %11[%arg5] [8] [1] : tensor<64xf32> to tensor<8xf32>
        %extracted_slice_7 = tensor.extract_slice %arg6[%arg4, %arg5] [%18, 8] [1, 1] : tensor<?x64xf32> to tensor<?x8xf32>
        %22 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%21, %extracted_slice_6 : tensor<?x8xf32>, tensor<8xf32>) outs(%extracted_slice_7 : tensor<?x8xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
        ^bb0(%in: f32, %in_8: f32, %out: f32):
          %23 = arith.addf %in, %in_8 : f32
          %24 = math.powf %23, %cst_3 : f32
          %25 = arith.mulf %24, %cst_2 : f32
          %26 = arith.addf %23, %25 : f32
          %27 = arith.mulf %26, %cst_1 : f32
          %28 = math.tanh %27 : f32
          %29 = arith.addf %28, %cst_0 : f32
          %30 = arith.mulf %23, %cst : f32
          %31 = arith.mulf %30, %29 : f32
          linalg.yield %31 : f32
        } -> tensor<?x8xf32>
        scf.forall.in_parallel {
          tensor.parallel_insert_slice %22 into %arg6[%arg4, %arg5] [%18, 8] [1, 1] : tensor<?x8xf32> into tensor<?x64xf32>
        }
      }
      scf.forall.in_parallel {
        tensor.parallel_insert_slice %17 into %arg3[%arg2, 0] [%16, 64] [1, 1] : tensor<?x64xf32> into tensor<?x64xf32>
      }
    }
    scf.forall.in_parallel {
