  ...
  %13 = scf.forall (%arg0) = (0) to (17) step (16) shared_outs(%arg1 = %12) -> (tensor<17x64xf32>) {
    ...
    %15 = scf.forall (%arg2) = (0) to (%14) step (16) shared_outs(%arg3 = %extracted_slice) -> (tensor<?x64xf32>) {
      ...
      %17 = scf.forall (%arg4, %arg5) = (0, 0) to (%16, 64) step (8, 8) shared_outs(%arg6 = %extracted_slice_5) -> (tensor<?x64xf32>) {
        ...
        %extracted_slice_6 = tensor.extract_slice %9[%19, 0] [%18, 32] [1, 1] : tensor<17x32xf32> to tensor<?x32xf32>
        %extracted_slice_7 = tensor.extract_slice %10[%arg5, 0] [8, 32] [1, 1] : tensor<64x32xf32> to tensor<8x32xf32>
        %20 = tensor.empty(%18) : tensor<?x8xf32>
        %21 = linalg.fill {stream.affinity = #hal.device.affinity<@__device_0>} ins(%cst_4 : f32) outs(%20 : tensor<?x8xf32>) -> tensor<?x8xf32>
        %22 = linalg.matmul indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>] {lowering_config = #iree_cpu.lowering_config<cache_parallel = [16, 0, 0], cache_reduction = [0, 0, 0], distribution = [16, 0, 0], vector_common_parallel = [8, 8, 0], vector_reduction = [0, 0, 1]>, stream.affinity = #hal.device.affinity<@__device_1>} ins(%extracted_slice_6, %extracted_slice_7 : tensor<?x32xf32>, tensor<8x32xf32>) outs(%21 : tensor<?x8xf32>) -> tensor<?x8xf32>
        %extracted_slice_8 = tensor.extract_slice %11[%arg5] [8] [1] : tensor<64xf32> to tensor<8xf32>
        %extracted_slice_9 = tensor.extract_slice %arg6[%arg4, %arg5] [%18, 8] [1, 1] : tensor<?x64xf32> to tensor<?x8xf32>
        %23 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%22, %extracted_slice_8 : tensor<?x8xf32>, tensor<8xf32>) outs(%extracted_slice_9 : tensor<?x8xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
        ...
        scf.forall.in_parallel {
          tensor.parallel_insert_slice %23 into %arg6[%arg4, %arg5] [%18, 8] [1, 1] : tensor<?x8xf32> into tensor<?x64xf32>
        }
      }
      scf.forall.in_parallel {
