...
module attributes {stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  ^bb1:  // pred: ^bb0
    ...
    %allocator_3 = hal.device.allocator<%__device_1_1 : !hal.device> : !hal.allocator
    %buffer = hal.allocator.allocate<%allocator_3 : !hal.allocator> affinity(%c-1_i64_2) type(%memory_types) usage(%buffer_usage_0) : !hal.buffer{%c8448}
    %__device_1_4 = util.global.load immutable @__device_1 : !hal.device
    %c-1_i64_5 = arith.constant -1 : i64
    %c0_i32 = arith.constant 0 : i32
    %memory_file = hal.ex.file.from_memory device(%__device_1_4 : !hal.device) affinity(%c-1_i64_5) access(Read) buffer(%buffer_cst : !util.buffer)[%c0 for %c8448] flags(%c0_i32) : !hal.file
    %__device_1_6 = util.global.load immutable @__device_1 : !hal.device
    %c-1_i64_7 = arith.constant -1 : i64
    %fence = hal.fence.create device(%__device_1_6 : !hal.device) flags("None") : !hal.fence
    hal.device.queue.read<%__device_1_6 : !hal.device> affinity(%c-1_i64_7) wait(%0) signal(%fence) source(%memory_file : !hal.file)[%c0_i64] target(%buffer : !hal.buffer)[%c0] length(%c8448) flags("None")
  ...
  ^bb1:  // pred: ^bb0
    ...
    %ordinal = hal.executable.export.ordinal target(@main_dispatch_0::@embedded_elf_riscv_32::@main_dispatch_0_matmul_17x64x32_f32) : index
    %c2_1 = arith.constant 2 : index
    %c1_2 = arith.constant 1 : index
    %c1_3 = arith.constant 1 : index
    hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%c2_1, %c1_2, %c1_3]) bindings([
      (%c0 : index)[%c0_0, %c2176], 
      (%arg2 : !hal.buffer)[%c0_0, %c8448], 
      (%c1 : index)[%c0_0, %c4352]
    ]) flags("None")
  ...
  util.func private @__main_memoize_lookup(%arg0: !hal.device, %arg1: i64) -> !hal.command_buffer {
    ...
    %0 = util.cmp.eq %arg0, %__device_1 : !hal.device
    %1 = scf.if %0 -> (!hal.command_buffer) {
      %__main_memoize_result_0_device_1 = util.global.load @__main_memoize_result_0_device_1 : !hal.command_buffer
  ...
  util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
    ...
    %c1 = arith.constant 1 : index
    %1 = util.call @__main_memoize_lookup(%__device_1_1, %c-1_i64_2) : (!hal.device, i64) -> !hal.command_buffer
    %fence_4 = hal.fence.create device(%__device_1_1 : !hal.device) flags("None") : !hal.fence
    hal.device.queue.execute.indirect<%__device_1_1 : !hal.device> affinity(%c-1_i64_2) wait(%fence) signal(%fence_4) commands(%1) bindings([
      (%buffer : !hal.buffer)[%c0_3, %c2176], 
      (%transient_buffer : !hal.buffer)[%c0_3, %c4352]
    ]) flags("None")
    %c-1_i32 = arith.constant -1 : i32
    ...
    %c17_7 = arith.constant 17 : index
    %c64 = arith.constant 64 : index
    %c0_8 = arith.constant 0 : index
    %view = hal.buffer_view.create buffer(%transient_buffer : !hal.buffer)[%c0_8, %c4352] shape([%c17_7, %c64]) type(%element_type_f32_6) encoding(%dense_row_major_5) : !hal.buffer_view
