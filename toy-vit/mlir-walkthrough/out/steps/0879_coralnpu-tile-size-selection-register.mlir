func.func @main_dispatch_0_matmul_17x64x32_f32() {
  ...
  %9 = linalg.matmul indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>] {compilation_info = #iree_codegen.compilation_info<lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [8, 8, 0], vector_reduction = [0, 0, 1]>, translation_info = <pipeline = CPUDoubleTilingExpert>>, stream.affinity = #hal.device.affinity<@__device_1>} ins(%4, %5 : tensor<17x32xf32>, tensor<64x32xf32>) outs(%8 : tensor<17x64xf32>) -> tensor<17x64xf32>
