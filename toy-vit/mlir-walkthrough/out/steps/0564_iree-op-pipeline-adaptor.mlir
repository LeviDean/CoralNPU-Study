...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
    ...
    %1 = flow.dispatch.workgroups(%0, %cst, %__hoisted_tensor_64xf32) : (tensor<17x32xf32>, tensor<64x32xf32>, tensor<64xf32>) -> tensor<17x64xf32> attributes {stream.affinity = #hal.device.affinity<@__device_1>} =
      ...
      %7 = linalg.fill {stream.affinity = #hal.device.affinity<@__device_0>} ins(%cst_0 : f32) outs(%6 : tensor<17x64xf32>) -> tensor<17x64xf32>
      %8 = linalg.matmul indexing_maps = [#map, #map1, #map2] {stream.affinity = #hal.device.affinity<@__device_1>} ins(%3, %4 : tensor<17x32xf32>, tensor<64x32xf32>) outs(%7 : tensor<17x64xf32>) -> tensor<17x64xf32>
      %9 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel"]} ins(%8, %5 : tensor<17x64xf32>, tensor<64xf32>) outs(%6 : tensor<17x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
      ^bb0(%in: f32, %in_6: f32, %out: f32):
        %10 = arith.addf %in, %in_6 : f32
        %11 = math.powf %10, %cst_1 : f32
        %12 = arith.mulf %11, %cst_2 : f32
        %13 = arith.addf %10, %12 : f32
        %14 = arith.mulf %13, %cst_3 : f32
        %15 = math.tanh %14 : f32
        %16 = arith.addf %15, %cst_4 : f32
        %17 = arith.mulf %10, %cst_5 : f32
