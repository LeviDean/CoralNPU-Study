...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  util.global private mutable @__hoisted_tensor_64xf32__storage_size : index
  util.global private mutable @__hoisted_tensor_64xf32__offset : index
  util.global private mutable @__hoisted_tensor_64xf32__length : index
  util.global private @__constant_tensor_64x32xf32 : !stream.resource<constant>
  util.global private mutable @__constant_tensor_64x32xf32__storage_size : index
  util.global private mutable @__constant_tensor_64x32xf32__offset : index
  util.global private mutable @__constant_tensor_64x32xf32__length : index
  util.initializer {
    ...
    %did_map, %result = stream.resource.try_map on(#hal.device.affinity<@__device_1>) %buffer_cst[%c0_0] : !util.buffer -> i1, !stream.resource<constant>{%c8448}
    %1:5 = scf.if %did_map -> (!stream.timepoint, !stream.resource<constant>, index, index, index) {
      scf.yield %0, %result, %c8448, %c0, %c8448 : !stream.timepoint, !stream.resource<constant>, index, index, index
    } else {
      %6 = stream.resource.alloc uninitialized on(#hal.device.affinity<@__device_1>) : !stream.resource<constant>{%c8448}
      %file = stream.file.constant on(#hal.device.affinity<@__device_1>) %buffer_cst[%c0_0 for %c8448] : !util.buffer{%c8448} -> !stream.file
      %7 = stream.file.read on(#hal.device.affinity<@__device_1>) await(%0) => %file[%c0_i64], %6[%c0_0], %c8448 : !stream.file -> !stream.resource<constant>{%c8448} => !stream.timepoint
      scf.yield %7, %6, %c8448, %c0, %c8448 : !stream.timepoint, !stream.resource<constant>, index, index, index
    }
    %2 = stream.resource.subview %1#1[%1#3] : !stream.resource<constant>{%1#2} -> !stream.resource<constant>{%1#4}
    %3 = stream.timepoint.await sync %1#0 => %2 : !stream.resource<constant>{%c8448}
    %4 = stream.resource.subview %3[%c256] : !stream.resource<constant>{%c8448} -> !stream.resource<constant>{%c8192}
    %5 = stream.resource.subview %3[%c0_0] : !stream.resource<constant>{%c8448} -> !stream.resource<constant>{%c256}
    util.global.store %3, @__constant_tensor_64x32xf32 : !stream.resource<constant>
    ...
    util.global.store %3, @__hoisted_tensor_64xf32 : !stream.resource<constant>
    util.global.store %c8448, @__hoisted_tensor_64xf32__storage_size : index
    util.global.store %c0_0, @__hoisted_tensor_64xf32__offset : index
  ...
  util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
    ...
    %__constant_tensor_64x32xf32__storage_size = util.global.load @__constant_tensor_64x32xf32__storage_size : index
    %__constant_tensor_64x32xf32__offset = util.global.load @__constant_tensor_64x32xf32__offset : index
    %__constant_tensor_64x32xf32__length = util.global.load @__constant_tensor_64x32xf32__length : index
    %0 = stream.resource.subview %__constant_tensor_64x32xf32[%__constant_tensor_64x32xf32__offset] : !stream.resource<constant>{%__constant_tensor_64x32xf32__storage_size} -> !stream.resource<constant>{%__constant_tensor_64x32xf32__length}
    %__hoisted_tensor_64xf32 = util.global.load immutable @__hoisted_tensor_64xf32 : !stream.resource<constant>
    %__hoisted_tensor_64xf32__storage_size = util.global.load @__hoisted_tensor_64xf32__storage_size : index
    %__hoisted_tensor_64xf32__offset = util.global.load @__hoisted_tensor_64xf32__offset : index
    %__hoisted_tensor_64xf32__length = util.global.load @__hoisted_tensor_64xf32__length : index
    %1 = stream.resource.subview %__hoisted_tensor_64xf32[%__hoisted_tensor_64xf32__offset] : !stream.resource<constant>{%__hoisted_tensor_64xf32__storage_size} -> !stream.resource<constant>{%__hoisted_tensor_64xf32__length}
    ...
    %3 = stream.timepoint.join max(%__hoisted_tensor_64xf32__timepoint_0, %__hoisted_tensor_64xf32__timepoint) => !stream.timepoint
    %result, %result_timepoint = stream.resource.alloca uninitialized on(#hal.device.affinity<@__device_1>) await(%3) => !stream.resource<external>{%c4352} => !stream.timepoint
    %4 = stream.timepoint.join max(%__hoisted_tensor_64xf32__timepoint_0, %__hoisted_tensor_64xf32__timepoint, %result_timepoint) => !stream.timepoint
    %5 = stream.cmd.execute on(#hal.device.affinity<@__device_1>) await(%4) => with(%2 as %arg1: !stream.resource<external>{%c2176}, %0 as %arg2: !stream.resource<constant>{%c8192}, %1 as %arg3: !stream.resource<constant>{%c256}, %result as %arg4: !stream.resource<external>{%c4352}) {
    ...
    %6 = stream.timepoint.await %5 => %result : !stream.resource<external>{%c4352}
    %7 = stream.tensor.export on(#hal.device.affinity<@__device_1>) %6 : tensor<17x64xf32> in !stream.resource<external>{%c4352} -> !hal.buffer_view
