...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  util.initializer {
    ...
    %results:2, %result_timepoint = stream.resource.constants on(#hal.device.affinity<@__device_1>) : 
      !stream.resource<constant>{%c256} = dense_resource<__elided__> : tensor<64xf32>,
      !stream.resource<constant>{%c8192} = dense_resource<torch_tensor_64_32_torch.float32> : tensor<64x32xf32>
      => !stream.timepoint
    %1 = stream.cmd.execute once on(#hal.device.affinity<@__device_1>) with() {
    } => !stream.timepoint
    %2 = stream.timepoint.join max(%result_timepoint, %1) => !stream.timepoint
    %3:2 = stream.timepoint.await sync %2 => %results#1, %results#0 : !stream.resource<constant>{%c8192}, !stream.resource<constant>{%c256}
    util.global.store %3#0, @__constant_tensor_64x32xf32 : !stream.resource<constant>
    util.global.store %3#1, @__hoisted_tensor_64xf32 : !stream.resource<constant>
