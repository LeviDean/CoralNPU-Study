...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  stream.executable private @main_dispatch_0 {
    ...
    builtin.module {
      func.func @main_dispatch_0_matmul_17x64x32_f32(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}, %arg2: !stream.binding {stream.alignment = 64 : index}, %arg3: index {stream.values = [0 : index]}, %arg4: index {stream.alignment = 256 : index, stream.values = [256 : index]}, %arg5: index {stream.values = [0 : index]}, %arg6: index {stream.values = [0 : index]}) {
        %0:4 = util.assume.int 
            %arg3<umin = 0, umax = 0>, 
            %arg4<umin = 256, umax = 256, udiv = 256>, 
            %arg5<umin = 0, umax = 0>, 
            %arg6<umin = 0, umax = 0>
        ...
        %c0 = arith.constant 0 : index
        %1 = stream.binding.subspan %arg0[%0#0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<17x32xf32>>
        %2 = stream.binding.subspan %arg1[%0#1] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x32xf32>>
        %3 = stream.binding.subspan %arg1[%0#2] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<64xf32>>
        %4 = stream.binding.subspan %arg2[%0#3] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<17x64xf32>>
        %5 = iree_tensor_ext.dispatch.tensor.load %1, offsets = [0, 0], sizes = [17, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<17x32xf32>> -> tensor<17x32xf32>
        %6 = iree_tensor_ext.dispatch.tensor.load %2, offsets = [0, 0], sizes = [64, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x32xf32>> -> tensor<64x32xf32>
        %7 = iree_tensor_ext.dispatch.tensor.load %3, offsets = [0], sizes = [64], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64xf32>> -> tensor<64xf32>
        %8 = tensor.empty() : tensor<17x64xf32>
        %9 = linalg.fill {stream.affinity = #hal.device.affinity<@__device_0>} ins(%cst_4 : f32) outs(%8 : tensor<17x64xf32>) -> tensor<17x64xf32>
        %10 = linalg.matmul indexing_maps = [#map, #map1, #map2] {stream.affinity = #hal.device.affinity<@__device_1>} ins(%5, %6 : tensor<17x32xf32>, tensor<64x32xf32>) outs(%9 : tensor<17x64xf32>) -> tensor<17x64xf32>
        %11 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel"]} ins(%10, %7 : tensor<17x64xf32>, tensor<64xf32>) outs(%8 : tensor<17x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
        ^bb0(%in: f32, %in_5: f32, %out: f32):
          %12 = arith.addf %in, %in_5 : f32
          ...
          %19 = arith.mulf %12, %cst : f32
          %20 = arith.mulf %19, %18 : f32
          linalg.yield %20 : f32
        } -> tensor<17x64xf32>
        iree_tensor_ext.dispatch.tensor.store %11, %4, offsets = [0, 0], sizes = [17, 64], strides = [1, 1] : tensor<17x64xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<17x64xf32>>
        return
      }
    }
  }
  util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
    ...
    %1 = stream.cmd.execute on(#hal.device.affinity<@__device_1>) await(%result_timepoint) => with(%0 as %arg1: !stream.resource<external>{%c2176}, %__hoisted_tensor_64xf32 as %arg2: !stream.resource<constant>{%c8448}, %result as %arg3: !stream.resource<external>{%c4352}) {
      stream.cmd.dispatch @main_dispatch_0::@main_dispatch_0_matmul_17x64x32_f32(%c0, %c256, %c0, %c0 : index, index, index, index) {
