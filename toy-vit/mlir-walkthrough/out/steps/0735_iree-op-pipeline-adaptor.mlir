...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  util.initializer {
    ...
    %1:2 = scf.if %did_map -> (!stream.timepoint, !stream.resource<constant>) {
      ...
      %5 = stream.resource.alloc uninitialized on(#hal.device.affinity<@__device_1>) : !stream.resource<constant>{%c8448}
      %file = stream.file.constant on(#hal.device.affinity<@__device_1>) %buffer_cst[%c0 for %c8448] : !util.buffer{%c8448} -> !stream.file
      %6 = stream.file.read on(#hal.device.affinity<@__device_1>) await(%0) => %file[%c0_i64], %5[%c0], %c8448 : !stream.file -> !stream.resource<constant>{%c8448} => !stream.timepoint
    ...
    %3 = stream.resource.subview %2[%c256] : !stream.resource<constant>{%c8448} -> !stream.resource<constant>{%c8192}
    %4 = stream.resource.subview %2[%c0] : !stream.resource<constant>{%c8448} -> !stream.resource<constant>{%c256}
    util.global.store %3, @__constant_tensor_64x32xf32 : !stream.resource<constant>
    util.global.store %4, @__hoisted_tensor_64xf32 : !stream.resource<constant>
