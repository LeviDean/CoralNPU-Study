util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
  ...
  ^bb0(%in: f32, %in_7: f32, %out: f32):
    %34 = arith.addf %in, %in_7 : f32
    linalg.yield %34 : f32
  } -> tensor<17x64xf32>
  %16 = linalg.fill ins(%cst_5 : f32) outs(%7 : tensor<f32>) -> tensor<f32>
  %17 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> ()>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%16 : tensor<f32>) outs(%4 : tensor<17x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  } -> tensor<17x64xf32>
  ...
  %19 = linalg.fill ins(%cst_6 : f32) outs(%7 : tensor<f32>) -> tensor<f32>
  %20 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> ()>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%19 : tensor<f32>) outs(%4 : tensor<17x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  } -> tensor<17x64xf32>
  ...
  ^bb0(%in: f32, %in_7: f32, %out: f32):
    %34 = math.powf %in, %in_7 : f32
    linalg.yield %34 : f32
  } -> tensor<17x64xf32>
  %22 = linalg.fill ins(%cst_2 : f32) outs(%7 : tensor<f32>) -> tensor<f32>
  %23 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> ()>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%22 : tensor<f32>) outs(%4 : tensor<17x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  } -> tensor<17x64xf32>
  ...
  %26 = linalg.fill ins(%cst_3 : f32) outs(%7 : tensor<f32>) -> tensor<f32>
  %27 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> ()>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%26 : tensor<f32>) outs(%4 : tensor<17x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  } -> tensor<17x64xf32>
