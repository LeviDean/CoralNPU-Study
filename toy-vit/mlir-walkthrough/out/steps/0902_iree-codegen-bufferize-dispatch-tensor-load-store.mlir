func.func @main_dispatch_0_matmul_17x64x32_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert>} {
  ...
  %8 = iree_codegen.load_from_buffer %0 : memref<17x32xf32, #hal.descriptor_type<storage_buffer>> -> tensor<17x32xf32>
  %9 = iree_codegen.load_from_buffer %2 : memref<64x32xf32, strided<[32, 1], offset: ?>, #hal.descriptor_type<storage_buffer>> -> tensor<64x32xf32>
  %10 = iree_codegen.load_from_buffer %4 : memref<64xf32, #hal.descriptor_type<storage_buffer>> -> tensor<64xf32>
  %11 = tensor.empty() : tensor<17x64xf32>
  %12 = scf.forall (%arg0) = (0) to (17) step (16) shared_outs(%arg1 = %11) -> (tensor<17x64xf32>) {
    %13 = affine.min affine_map<(d0) -> (-d0 + 17, 16)>(%arg0)
    %extracted_slice = tensor.extract_slice %8[%arg0, 0] [%13, 32] [1, 1] : tensor<17x32xf32> to tensor<?x32xf32>
    %14 = tensor.empty(%13) : tensor<?x64xf32>
    %15 = linalg.fill {stream.affinity = #hal.device.affinity<@__device_0>} ins(%cst : f32) outs(%14 : tensor<?x64xf32>) -> tensor<?x64xf32>
    %16 = linalg.matmul indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>] {lowering_config = #iree_cpu.lowering_config<cache_parallel = [16, 0, 0], cache_reduction = [0, 0, 0], distribution = [16, 0, 0], vector_common_parallel = [8, 8, 0], vector_reduction = [0, 0, 1]>, stream.affinity = #hal.device.affinity<@__device_1>} ins(%extracted_slice, %9 : tensor<?x32xf32>, tensor<64x32xf32>) outs(%15 : tensor<?x64xf32>) -> tensor<?x64xf32>
    %extracted_slice_5 = tensor.extract_slice %arg1[%arg0, 0] [%13, 64] [1, 1] : tensor<17x64xf32> to tensor<?x64xf32>
    %17 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%16, %10 : tensor<?x64xf32>, tensor<64xf32>) outs(%extracted_slice_5 : tensor<?x64xf32>) attrs =  {stream.affinity = #hal.device.affinity<@__device_0>} {
    ...
    scf.forall.in_parallel {
      tensor.parallel_insert_slice %17 into %arg1[%arg0, 0] [%13, 64] [1, 1] : tensor<?x64xf32> into tensor<17x64xf32>
    }
  } {mapping = [#iree_codegen.workgroup_mapping<x>]}
  iree_codegen.store_to_buffer %12, %6 : tensor<17x64xf32> into memref<17x64xf32, #hal.descriptor_type<storage_buffer>>
  return
}
