...
^bb1:  // pred: ^bb0
  ...
  %allocator_2 = hal.device.allocator<%__device_1_1 : !hal.device> : !hal.allocator
  %buffer = hal.allocator.allocate<%allocator_2 : !hal.allocator> affinity(%c-1_i64) type(%memory_types) usage(%buffer_usage_0) : !hal.buffer{%c8448}
  %__device_1_3 = util.global.load immutable @__device_1 : !hal.device
  %memory_file = hal.ex.file.from_memory device(%__device_1_3 : !hal.device) affinity(%c-1_i64) access(Read) buffer(%buffer_cst : !util.buffer)[%c0 for %c8448] flags(%c0_i32) : !hal.file
  %__device_1_4 = util.global.load immutable @__device_1 : !hal.device
  %fence = hal.fence.create device(%__device_1_4 : !hal.device) flags("None") : !hal.fence
  hal.device.queue.read<%__device_1_4 : !hal.device> affinity(%c-1_i64) wait(%0) signal(%fence) source(%memory_file : !hal.file)[%c0_i64] target(%buffer : !hal.buffer)[%c0] length(%c8448) flags("None")
