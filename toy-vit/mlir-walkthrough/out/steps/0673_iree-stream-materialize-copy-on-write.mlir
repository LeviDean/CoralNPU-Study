util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
  ...
  %0 = stream.async.clone on(#hal.device.affinity<@__device_1>) %__constant_tensor_64x32xf32 : !stream.resource<constant>{%c8192} -> !stream.resource<*>{%c8192}
  %1 = stream.async.clone on(#hal.device.affinity<@__device_1>) %__hoisted_tensor_64xf32 : !stream.resource<constant>{%c256} -> !stream.resource<*>{%c256}
  ...
  %4 = stream.async.dispatch on(#hal.device.affinity<@__device_1>) @main_dispatch_0::@main_dispatch_0_matmul_17x64x32_f32(%3[%c0 to %c2176 for %c2176], %0[%c0 to %c8192 for %c8192], %1[%c0 to %c256 for %c256]) : (!stream.resource<*>{%c2176}, !stream.resource<*>{%c8192}, !stream.resource<*>{%c256}) -> !stream.resource<*>{%c4352}
