...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
    ...
    %1 = stream.timepoint.join max(%__constant_tensor_64x32xf32__timepoint, %__hoisted_tensor_64xf32__timepoint) => !stream.timepoint
    %results, %result_timepoint = stream.async.execute on(#hal.device.affinity<@__device_1>) await(%1) => with(%0 as %arg1: !stream.resource<external>{%c2176}, %__constant_tensor_64x32xf32 as %arg2: !stream.resource<constant>{%c8192}, %__hoisted_tensor_64xf32 as %arg3: !stream.resource<constant>{%c256}) -> !stream.resource<external>{%c4352} {
      %4 = stream.async.dispatch @main_dispatch_0::@main_dispatch_0_matmul_17x64x32_f32(%arg1[%c0 to %c2176 for %c2176], %arg2[%c0 to %c8192 for %c8192], %arg3[%c0 to %c256 for %c256]) : (!stream.resource<external>{%c2176}, !stream.resource<constant>{%c8192}, !stream.resource<constant>{%c256}) -> !stream.resource<external>{%c4352}
      stream.yield %4 : !stream.resource<external>{%c4352}
    } => !stream.timepoint
    %2 = stream.timepoint.await %result_timepoint => %results : !stream.resource<external>{%c4352}
