...
module attributes {iree.fixedpoint.iteration = 0 : index, iree.fixedpoint.modified, stream.affinity.default = #hal.device.affinity<@__device_0>, stream.topology = #hal.device.topology<links = [(@__device_0 -> @__device_1 = {unified_memory = true, transparent_access = true}), (@__device_1 -> @__device_0 = {unified_memory = true, transparent_access = true})]>} {
  ...
  util.func public @main(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @main(%input0: tensor<17x32xf32>) -> (%output0: tensor<17x64xf32>)"}} {
    ...
    %1 = stream.tensor.import on(#hal.device.affinity<@__device_1>) %arg0 : !hal.buffer_view -> tensor<17x32xf32> in !stream.resource<external>{%c2176}
    %result, %result_timepoint = stream.resource.alloca uninitialized on(#hal.device.affinity<@__device_1>) await(%0) => !stream.resource<external>{%c4352} => !stream.timepoint
    %2 = stream.timepoint.join max(%0, %result_timepoint) => !stream.timepoint
    %3 = stream.cmd.execute on(#hal.device.affinity<@__device_1>) await(%2) => with(%1 as %arg1: !stream.resource<external>{%c2176}, %__hoisted_tensor_64xf32 as %arg2: !stream.resource<constant>{%c8448}, %result as %arg3: !stream.resource<external>{%c4352}) {
